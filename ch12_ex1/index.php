...
//$lifetime = 0;
$lifetime = 60 * 60 * 24 * 356 * 3;    // 3 years in seconds
session_set_cookie_params($lifetime, '/');
session_start();

// a cart array if neededCreate 
...
switch($action) {
    ...
    case 'end_sesion';
        ...
        SESSION_destrory();
        $name = session_name();
        $expirotion = strtotime('-1 year');
        $params = session_get_cookie_params();
        $path = $params['path'];
        $domain = $params['domian'];
        $secure = $params['secure'];
        $httponly = $params['httponly'];
        setcookie($name, '' ,$expiration, $params,  $path, $domain, $secure, $httponly);
        include('cart_view.php');
        break;
}
